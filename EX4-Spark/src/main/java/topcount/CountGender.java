...
public class CountGender {
    //统计双十一购买了商品的男女比例
    /* 统计男女比例，查询条件：user_log表左连接user_info表，
    日期=1111，action_type=2，按性别分组，
    统计不重复的user_id数，还要考虑排除异常的gender值。 */
    public static void main(String[] args) throws Exception  {
        ...
        SparkSession spark = SparkSession.builder().appName("CountGender").getOrCreate();
        ...
        //创建info_pair，key是user_id，value是性别
        ...
        JavaPairRDD<String,String> info_pairs = info_words.mapToPair(
            s -> {
                return new Tuple2<String,String>(s.get(0),s.get(2));
            }
        );

        //然后将两个pair连接，形成PairRdd:<user_id,(item_id,gender)>
        JavaPairRDD<String,Tuple2<String,String>> joined_logs = log_pairs.join(info_pairs);
        //修改PairRdd的格式，改成<user_id,gender>
        ...
        JavaPairRDD<String,Integer> gender_ones = user_gender.mapToPair(
            s -> {
                return new Tuple2<String,Integer>(s._2,1);
            }
        );
        //然后按性别将计数加总
        JavaPairRDD<String, Integer> counts = gender_ones.reduceByKey((i1, i2) -> i1 + i2);
        List<Tuple2<String, Integer>> output = counts.collect();
        List<Tuple2<String, Integer>> countable = new ArrayList<Tuple2<String, Integer>>();
        List<String> genders = Arrays.asList(new String[] {"0", "1"});
        int total_num = 0;
